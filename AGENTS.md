...
- `scripts/toolcribEdrawingsIso.ts` — CAD source (eDrawings/SolidWorks) → catalog. Produces both the ISO (isometric, from the 3D model) and the CAD (dimensioned plan, from a `.pdf` companion or a `.slddrw` export in flat mode) for each piece. See "Learned Workspace Facts" below for the full contract.
...
Path: `C:\Users\emili\Codex-obsidian`
...
- Biblioteca agrupa CAD+ISO en una fila por pieza (`toolcribCatalog.ts`/`groupDrawingViews`; `canonicalPartNumber` quita `.ISO`); los ISO ya no se ocultan del listado. Vista página full-viewport (`variant="page"`) con chips de activos (CAD/ISO/3D/Sin PDF). Imprimir OT sigue siendo solo CAD (`printDrawingForGroup` en Biblioteca; `selectCadDrawingForPrint` en Órdenes); `excludeIsoForPrint` solo afecta orden de búsqueda al adjuntar en Reporte.
- Order–Drawing Bridge es estado de sesión en `App` (no Firestore): vincula línea de orden ↔ plano CAD/ISO para Imprimir OT y Enviar a reporte; el envío desde Órdenes puede auto-disparar auditoría. Reporte expone provenance (ISO eDrawings/CAD/IA), discrepancia de revisión, visor STL (`stlUrl`), match por alias y badge COMPRAR por sesión.
- Herramental (`ToolingHubView`, NavRail): calculadora S&F, asesor de planos (Gemini vía `analyzeGemini`), endmills, decodificador ISO, grados Haas, directorio de proveedores y Mi Bóveda. Firestore `toolingPurchases` (CRUD autenticado, misma regla que `purchases`).
- Vision no abre `.sldprt`/SolidWorks en el navegador: todo plano nuevo necesita `pdfUrl` en Storage. STL en catálogo solo con `toolcrib:edrawings-iso --includeStl` o `toolcrib:upload-stls`; sin `stlUrl` no hay Ver 3D — el reporte usa el PDF ISO para la cara isométrica, no el STL.
- Pipeline eDrawings: `npm run toolcrib:edrawings-iso` genera `{PART}.ISO.pdf` (isométrica, `-Mode iso`) desde el modelo 3D (`.sldprt`/`.easm`/`.eprt`) y, en paralelo por pieza, `{PART}.pdf` (CAD acotado) desde su companion `.pdf` real si existe, o exportado en `-Mode flat` (sin isométrica forzada, solo zoom-to-fit) desde el `.slddrw` companion. `--skipCad` omite esta segunda parte. CAD e ISO son partIds distintos (`{PART}` vs `{PART}.ISO`) que el cliente ya agrupa por `canonicalPartNumber`/`groupDrawingViews` (`src/lib/toolcribCatalog.ts`) — sin cambio de esquema. Por defecto exporta sólo JPG/PDF para que un diálogo STL no bloquee lotes; `--includeStl` habilita STL explícitamente y fuerza una exportación fresca si hay exportador, sin adoptar STL residuales del `workDir`. Al reutilizar el mismo `--workDir`, el modo normal reanuda sólo desde JPEG completos con sidecar `.jpg.source.json` cuyos hashes SHA-256 coinciden con la fuente CAD y el JPEG (misma regla para el JPEG del CAD). El fallback Gemini genera isométrica si solo hay CAD (badge IA); el catálogo puede exponer `stlUrl` con visor 3D en Biblioteca. Plomería Firestore/Storage compartida (docIds, upload, upsert-con-merge) vive en `scripts/toolcrib/lib/firestoreCatalog.ts`; matching STL↔drawing (con veto de identificador exacto, sin contención de substring) en `scripts/toolcrib/lib/stlMatch.ts`. `npm run toolcrib:audit` reporta por pieza qué le falta (CAD/ISO/STL) reusando el mismo agrupador que la Biblioteca.
- CAD huérfano (`discoverCadOnlySources`): algunas piezas solo tienen `.slddrw` (y a veces `.pdf`), sin ningún modelo 3D (`.sldprt`/`.easm`/`.eprt`) — invisibles para el descubrimiento normal, que camina a partir de archivos de modelo. `toolcribEdrawingsIso.ts` corre una segunda pasada de descubrimiento solo-CAD sobre el mismo `--scan`, excluyendo `basePartNumber`s ya cubiertos por el flujo normal; a esas piezas no les hace ISO (no hay de dónde), solo CAD. No respeta `--limit` (barato, no compite por el cupo del batch de isométricas). Verificado contra el catálogo real de SUPRAJIT 2026-08-26: subió CAD a 46 piezas (41 con modelo 3D + 5 solo-CAD), 0 fallos; el JPEG del CAD vive en una subcarpeta `cad/` propia dentro de `{basePartNumber}/` para no pisar el JPEG de la ISO cuando el `.sldprt` y su `.slddrw` companion comparten stem (caso típico).
- Windows MAX_PATH (260 car.): el esquema de promoción atómica de `Export-EDrawings.ps1` (`.{stem}.{guid32hex}.jpg.partial`) puede exceder el límite clásico si `--workDir` está muy anidado y el "número de parte" es un nombre descriptivo largo (p.ej. los punzones "PUNZONES DE MARCA-SUPRAJIT SMV-0XX LETRA Y", ~44 caracteres) — el síntoma es `inventory-only`/`No se puede encontrar una parte de la ruta de acceso` sin que sea un fallo real del exportador. Usar un `--workDir` corto (`./scratch/...`) evita el problema en la enorme mayoría de casos.
