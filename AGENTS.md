...
- Pipeline eDrawings: `npm run toolcrib:edrawings-iso` genera `{PART}.ISO.pdf` desde companions; por defecto exporta sólo JPG/PDF para que un diálogo STL no bloquee lotes. `--includeStl` habilita STL explícitamente y fuerza una exportación fresca si hay exportador, sin adoptar STL residuales del `workDir`. Al reutilizar el mismo `--workDir`, el modo normal reanuda sólo desde JPEG completos con sidecar `.jpg.source.json` cuyos hashes SHA-256 coinciden con la fuente CAD y el JPEG. El fallback Gemini genera isométrica si solo hay CAD (badge IA); el catálogo puede exponer `stlUrl` con visor 3D en Biblioteca.
