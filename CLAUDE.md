...
`App.tsx` wires views and passes callbacks; all analysis state and the `extractInfo()` flow live in the `useVisionAnalysis` hook, including edit-mode state (edit/exclude/restore orders, cantidad writes to Firestore) and results-display state (filter, search, preview — `filteredResults`). The pipeline:

1. **Order extraction** — Orders are read from Firestore (`odooSaleOrders`, synced by `scripts/syncOdoo.ts`) via `listOrdersToInvoice()`; lines with no pending quantity are skipped and the part number is parsed from the `[12345] Item` bracket prefix. No Gemini call is involved in this step (`ORDER_PROMPT_VERSION` survives only as an IndexedDB cache key).
...
4. **Blueprint analysis** — The JPEG is sent to `gemini-3.5-flash` Vision; it returns `BlueprintSpec[]` with piece label and an isometric bounding box (`[ymin, xmin, ymax, xmax]` on a 0–1000 grid). Results are cached in IndexedDB keyed by `BLUEPRINT_PROMPT_VERSION`. **Two-pass refinement**: if the initial bounding box area exceeds `REFINEMENT_SKIP_AREA_THRESHOLD = 400_000`, a second Gemini call crops the region and re-asks for a tighter box, then maps coordinates back to the original 0–1000 space.
