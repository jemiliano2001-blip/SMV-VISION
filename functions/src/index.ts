 ...
 *
 * Cloud Function programada (cada 30 min) que sincroniza las órdenes de
 * SUPRAJIT desde Odoo 15 hacia Firestore. Es el equivalente automático del
 * script manual `scripts/syncOdoo.ts` y replica su lógica probada:
 *
 ...
 *   5. Upsert de órdenes de trabajo → colección `workOrders` (tablero Control),
 *      deduplicadas por `SO::parte`, archivando las que ya no aplican.
...
// ─────────────────────────────────────────────────────────────────────────────
//  Tipos de dominio (forma intermedia tras el ensamblado desde Odoo)
// ─────────────────────────────────────────────────────────────────────────────

interface OdooDeliveryLine {
  product: string;
  qty_demand: number;
  qty_done: number;
  state: string;
...
interface OdooOrderLine {
  ...
  /** Calculada desde stock.move (traslados pendientes). */
...
/** Picking con su id interno conservado para amarrar los stock.move. */
interface OdooPicking extends OdooDelivery {
  id: number;
  origin: string;
}
 ...
 */
function parseOdooProduct(
  product: string,
): {numeroParte: string; pieza: string} {
...
function connectOdoo(odoo: Odoo): Promise<void> {
...
async function fetchSaleOrders(odoo: Odoo): Promise<OdooSaleOrder[]> {
...
async function fetchStockMoves(
  odoo: Odoo,
  pickingIds: number[],
): Promise<Map<number, OdooDeliveryLine[]>> {
  const map = new Map<number, OdooDeliveryLine[]>();
 ...
 */
function computePendingQuantities(order: OdooSaleOrder): void {
...
async function upsertSaleOrders(
...
// ─────────────────────────────────────────────────────────────────────────────
//  Upsert → workOrders (órdenes de trabajo; tablero Control)
// ─────────────────────────────────────────────────────────────────────────────

interface WorkOrderSyncResult {
  created: number;
  updated: number;
  archived: number;
}
...
async function upsertWorkOrders(
  ...
  const toArchive = odooOts.docs.filter((d) => {
    ...
    const numeroParte =
      typeof raw["numeroParte"] === "string" ? raw["numeroParte"] : "";
    const pieza = typeof raw["pieza"] === "string" ? raw["pieza"] : "";
    const poNumber =
      typeof raw["poNumber"] === "string" ? raw["poNumber"] : "";
  ...
  for (const order of pendingOrders) {
    ...
    const otDate =
      order.date_order !== false ? order.date_order.split(" ")[0] : "";
  ...
  const allOps = [
    ...toCreate.map((payload) => ({type: "create" as const, payload})),
...
// ─────────────────────────────────────────────────────────────────────────────
//  Núcleo del sync (exportado para poder ejecutarlo/probarlo fuera del scheduler)
// ─────────────────────────────────────────────────────────────────────────────
...
export const syncSuprajitOrders = onSchedule(
  ...
  async () => {
    ...
    try {
      ...
      const password = ODOO_API_KEY.value();
