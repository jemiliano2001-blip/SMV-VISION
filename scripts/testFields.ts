...
async function run() {
  ...
  for (const order of orders) {
    ...
    const pickings = await xmlRpcCall('object', 'execute_kw', readParams);
    for (const p of pickings) {
      console.log('Picking:', p.name);
      const moveParams = [
        odooDB, uid, odooApiKey,
        'stock.move', 'search_read',
        [[['picking_id', '=', p.id]]],
        { fields: ['id', 'product_id', 'sale_line_id', 'description_picking', 'name'] }
      ];
      const moves = await xmlRpcCall('object', 'execute_kw', moveParams);
