...
export function WorkOrdersPanel({ initialAlertFilter = null, onDataChanged }: WorkOrdersPanelProps = {}): ReactElement {
  ...
  // ── tarjeta de orden (reutilizada por tablero y lista) ──────────────────────

  const OrderCard = (o: WorkOrder): ReactElement => {
            ...
            <div className="flex items-center gap-2 flex-wrap">
              {isEditingDueDate ? (
                ...
                >
                  ...
                  <button type="button" onClick={() => setEditingDueDateId(null)} className="px-2 py-0.5 border border-line text-[9px] font-black uppercase text-ink-dim">✕</button>
                </form>
              ) : (
                <button
                  type="button"
                  onClick={() => { setEditingDueDateId(o.id); setDraftDueDate(o.dueDate ?? ''); }}
          ...
          <div className="flex flex-col items-stretch gap-1.5 shrink-0">
            ...
            {o.status === 'pendiente' && (
              <select
                ...
                onChange={(e) => { const v = e.target.value; if (v) void handleTransition(o, 'en_proceso', v); e.currentTarget.value = ''; }}
            ...
            {o.status === 'terminada' && (
              <>
                <select
                  defaultValue=""
                  disabled={!!busy || activeTorneros.length === 0}
                  onChange={(e) => { const v = e.target.value; if (v) void handleTransition(o, 'entregada', v); e.currentTarget.value = ''; }}
