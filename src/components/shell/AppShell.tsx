...
export function AppShell({ activeView, onNavigate, version, children }: AppShellProps): ReactElement {
  ...
  return (
    <div className="h-screen w-full flex bg-bg text-ink overflow-hidden font-sans">
      {/* Desktop Sidebar */}
      <div className="hidden lg:block w-[248px] shrink-0 border-r border-line shadow-[6px_0_24px_rgba(15,23,42,0.04)] z-20">
      ...
      <main className="flex-1 min-w-0 h-full flex flex-col relative overflow-hidden">
        {/* Mobile Header */}
        <div className="lg:hidden shrink-0 h-14 sm:h-16 border-b border-line bg-surface/95 backdrop-blur-xl flex items-center justify-between px-3 sm:px-4 gap-2 z-20">
          <div className="flex items-center gap-2.5 min-w-0">
            <Sheet open={mobileDrawerOpen} onOpenChange={setMobileDrawerOpen}>
              <SheetTrigger render={
                <Button
                  variant="outline"
                  size="icon"
                  className="h-10 w-10 rounded-lg border border-line text-ink bg-surface-2 hover:bg-accent hover:border-accent hover:text-white transition-colors shrink-0"
                  aria-label="Abrir menú de navegación"
                >
                  <Menu size={18} />
                </Button>
              } />
              <SheetContent side="left" className="p-0 w-64 border-r border-line bg-surface">
            ...
            <div className="flex items-center gap-2 min-w-0">
              <div className="font-display font-bold text-lg sm:text-xl tracking-[-0.5px] leading-none pt-0.5 shrink-0">
                SMV<span className="text-accent">/</span>VISION
              </div>
              <span className="hidden xs:inline-block text-ink-dim">·</span>
              <span className="font-sans text-[11px] sm:text-xs font-semibold text-ink-dim truncate max-w-[130px] sm:max-w-[200px]">
          ...
          <div className="flex items-center gap-1.5 shrink-0">
            <button
              ...
              className="grid place-items-center size-11 rounded-lg border border-line bg-surface-2 text-ink-dim hover:text-accent transition-colors"
